import type { MigrationRolloutCheckpointEvent } from "@/lib/types";

type MigrationRolloutCheckpointTableProps = {
  events: MigrationRolloutCheckpointEvent[];
};

function formatDecision(decision: MigrationRolloutCheckpointEvent["decision"]) {
  return decision.replaceAll("_", " ");
}

export function MigrationRolloutCheckpointTable({
  events,
}: MigrationRolloutCheckpointTableProps) {
  if (!events.length) {
    return (
      <div className="rounded-[20px] border border-dashed border-[rgba(152,164,189,0.18)] px-5 py-6 text-sm text-[var(--text-secondary)]">
        No rollout-wave or scaling checkpoint decisions have been recorded yet.
      </div>
    );
  }
      ...
      <table className="min-w-full divide-y divide-[rgba(152,164,189,0.08)] text-left text-sm">
        <thead className="bg-[rgba(8,11,18,0.82)] text-[var(--text-muted)]">
          <tr>
            ...
            <th className="px-4 py-3 font-medium">Rollout snapshot</th>
