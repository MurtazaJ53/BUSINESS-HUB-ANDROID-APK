...
export function MigrationRunbookPanel({
  readiness,
}: MigrationRunbookPanelProps) {
  const readyCandidates = readiness.filter((row) => row.ready_for_pilot);
  const nextCandidate =
    readyCandidates.find(
      (row) => row.recommended_next_status === "postgres_primary",
    ) ??
    readyCandidates.find((row) => row.recommended_next_status === "ready") ??
    null;
  const blockedCandidates = readiness
    .filter((row) => !row.ready_for_pilot)
    .slice(0, 3);
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-bold">
            How to execute the next safe cutover
          </h2>
          <p className="mt-2 max-w-3xl text-sm text-[var(--text-secondary)]">
            This is the operator checklist for the first real Phase 3 pilot. It
            turns the migration console from a diagnostics page into an
            execution surface by showing which domain is actually safe to move
            and what to do before and after promotion.
          </p>
        ...
        <div className="grid gap-4">
          <div className="rounded-[24px] border border-[rgba(52,211,153,0.18)] bg-[rgba(7,33,25,0.72)] px-5 py-5">
            <p className="eyebrow text-[rgba(173,241,212,0.7)]">
            ...
            {nextCandidate ? (
              ...
              <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">
                No domain is currently ready for promotion. Clear blockers in
                the readiness table, rerun compare jobs, and use the
                reconciliation queue before attempting a pilot.
              </p>
            )}
          </div>

          <div className="rounded-[24px] border border-[rgba(251,113,133,0.16)] bg-[rgba(40,12,19,0.6)] px-5 py-5">
            <p className="eyebrow text-[rgba(251,113,133,0.72)]">
            ...
            <div className="mt-4 space-y-3">
              {blockedCandidates.length ? (
                ...
                <p className="text-sm leading-6 text-[var(--text-secondary)]">
                  No blocking domains at the moment. Keep watching bridge
                  receipts and compare summaries while pilots advance.
                </p>
              )}
            </div>
          </div>
